...
export default async function handler(req, res) {
  // CORS for localhost + your prod domain
  res.setHeader("Access-Control-Allow-Origin", "*"); // you can swap * for http://localhost:5174 and https://smart-study-journal.vercel.app
...
Return ONLY JSON array with objects: [{"front":"…","back":"…"}] (length 4). No commentary, no code fences.`
        : action === "explain"
    ...
    if (!r.ok) {
      ...
      return res.status(r.status).json({ error: t || `OpenAI error ${r.status}` });
    }
    ...
    if (action === "quiz" || action === "flashcards") {
      let parsed;
      try { parsed = JSON.parse(stripFences(content)); } catch { parsed = []; }
